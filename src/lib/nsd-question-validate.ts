...
const TEMPLATE_EXTRA: Record<QuestionKind, Record<string, unknown>> = {
  choice: {
    choices: ["選択肢1", "選択肢2", "選択肢3", "選択肢4"],
    answerIndex: 0,
  },
  blank: {
    template: "ここに ___ を含む文章を書く",
    accepted: [["正解1", "正解2"]],
  },
  order: {
    items: ["1番目の要素", "2番目の要素", "3番目の要素"],
  },
  bugfix: {
    starter: "ここに直す前のコードを書く",
    language: "html",
    mustInclude: ["含むべき文字列"],
  },
  code: {
...
/** A pretty-printed starting point for a new question of the given kind. */
export function newQuestionTemplate(kind: QuestionKind, difficulty: Difficulty): string {
  const id = `new-${kind}-${Date.now().toString(36)}`;
  const base = {
    id,
    difficulty,
    category: "HTML",
    prompt: "ここに問題文を書いてください",
    explanation: "ここに解説を書いてください",
    xp: 50,
  };
  return JSON.stringify({ ...base, kind, ...TEMPLATE_EXTRA[kind] }, null, 2);
}
